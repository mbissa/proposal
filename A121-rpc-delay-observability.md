...
To measure client-side delays, we introduce a duration histogram and a tracing span at the **Call Level** (for name resolution delays), and another histogram and span at the **Attempt Level** (for load balancing pick delays).
...
The prepending and wrapping logic is handled entirely inside the balancer picker tree, keeping the channel's attempt-routing wrapper and tracer plugins decoupled:
1.  **Leaf Pickers** (such as `pick_first` or `round_robin`): Generate the base leaf types (e.g., `delay_type = "connecting"`) and the initial `delay_reason` (e.g., `"subchannel connecting: TCP handshake in progress"`).
